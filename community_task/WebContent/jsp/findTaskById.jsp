...
<body>
						...
						<c:if test="${empty findMessage}">
							<div class='title1'>
								...
								<c:if test="${task.state==0 }">
									...
									<div class="task-btn task-state-bg2">已接取</div>
									
								</c:if> <c:if test="${task.state==2}">
									<div class="task-btn task-state-bg2">已完成</div>
								</c:if>
							</div>
							<div class="title2">
								<sapn class="task-money">
									悬赏：&ensp;${task.task_money}
									<span style="float:right;">${jiequ_message }</span>
								</sapn>
								...
								<c:if test="${task.state==2}">
									<div class="task-end_user">
										完成人：&ensp;${task.end_user.uname}
									</div>
									<div class="task-end_time">
								...
								<c:if test="${(task.state==1||task.state==2)&&(userinfo.uid eq task.start_user.uid||userinfo.uid == task.end_user.uid)}">
									<div class="task-end_user">
										发布人联系方式：&ensp;${task.start_user_tel}
									</div>
									<div class="task-end_time">
										接取人联系方式：&ensp;${task.end_user_tel}
									</div>
								</c:if>
							</div>
							...
							<c:if test="${userinfo.uid eq task.start_user.uid }">
								<div>
										<a href="${basePath }TaskServlet?type=update&tid=${task.tid}" class="task-btn task-btn-main" id="update">修改任务</a>
									<c:if test="${task.state==1}">
	...
	<!--弹出窗口--接取-->
	<div class="hiddendiv2" id="receive">
		<form action="${basePath }TaskServlet?type=update_jiequ&tid=${task.tid}" method="post">
	...
	<!--弹出窗口--拒接-->
	<div class="hiddendiv" id="hiddeDiv"></div>
